...
## 三、人工确认与系统原生弹窗规范（macOS AppleScript）

在 Claude 中，凡涉及**合并主干（dev/master/main）、推送远端、落地代码（land/promote）、执行 DDL/SQL 迁移或生产发布等关键/高危操作**，必须通过系统原生弹窗（`/usr/bin/osascript`）向用户请求确认，严禁静默执行或仅在聊天会话中假定已授权。
